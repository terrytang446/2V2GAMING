...
def save_model(model):
    ...
    if isinstance(model, Planner):
        return save(model.state_dict(), path.join(path.dirname(path.abspath(__file__)), 'planner2.th'))
...
def load_model():
    ...
    from os import path
    r = Planner()
    r.load_state_dict(load(path.join(path.dirname(path.abspath(__file__)), 'planner2.th'), map_location='cpu'))
    return r